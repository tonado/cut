...
#define ASSERT_BETWEEN(num, min, max)   \
    ...
    {
        CUI_OVERRIDE(void exec(Point&, Orientation&) const) {}
    };
}
...
namespace
{
    ...
    private:
        CUI_OVERRIDE(void exec(Point&, Orientation& orientation) const)
        {
...
namespace
{
    ...
    private:
        CUI_OVERRIDE(void exec(Point& point, Orientation& orientation) const)
        {
